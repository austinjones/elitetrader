...
pub struct SystemJson {
    ...
    pub updated_at: u64, //	faction: Option<String>,
...
pub fn get_stations_by_system(stations: &Vec<StationJson>) -> HashMap<u32, Vec<&StationJson>> {
    ...
    for s in stations.iter() {
        match result.get_mut(&s.system_id) {
            None => {
                result.insert(s.system_id, Vec::new());
            }
            _ => {}
        };
...
pub fn get_listings_by_station(
    listings: &Vec<StationCommodityListingJson>,
) -> HashMap<u32, Vec<&StationCommodityListingJson>> {
    let mut result = HashMap::new();

    for s in listings.iter() {
        match result.get_mut(&s.station_id) {
            None => {
                result.insert(s.station_id, Vec::new());
            }
            _ => {}
        };
