...
impl EdceData {
	...
	pub fn generate(basePath: &String) -> Option<EdceData> {
		match Command::new("python.exe")
			...
			Err(text) => {println!("EDCE Error: Could not execute command path: {}", text); None}
		}
	}
...
//		};
		...
		match File::open(&jsonPath) {
			...
			Err(text) => {println!("EDCE Error: Could not open last.json ({}): {}", text, &jsonPath); None}
		}
	}
	
	pub fn apply_edce_adjustments<'a>( &'a self, universe: &'a mut Universe ) -> Option<EdceStationUpdate> {
		...
		if let Some(mut station) = universe.get_station_by_name_mut( &self.lastSystem.name, &self.lastStarport.name ) {
			'listing: for listing in station.listings.iter_mut() {
				'commodity: for commodity in commodities {
					...
					if buy_price_delta.abs() > 0
						...
						let adjustment = PriceAdjustment::new(commodity.stock as u32, commodity.buyPrice, 
							commodity.sellPrice, listing);
						adjustment.save();
