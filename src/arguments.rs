...
//			.unwrap_or( prompt_value( "current system name" ) );
			...
			None => {
				match edce_data {
					&Some(ref edce) => {
						...
						println!("Loaded EDCE data - credit balance:\t{}", 
		...
		let minimum_balance_in = match config.opt_str("m") {
			Some(v) => v,
			None => prompt_value( "m", "minimum credit balance - saftey net for rebuy (e.g. 3.2m)" )
		};
		...
		let ship_size_in = match config.opt_str("p") {
			Some(v) => v,
			None => {
				match Self::get_ship_size(edce_data) {
					Some(v) => {
...
//		let username_in = prompt_value( "_", "elite dangerous username" );
//		let password_in = prompt_value( "_", "elite dangerous password" );
