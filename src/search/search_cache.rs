...
use crate::search::search::SearchStation;
...
impl CachedTrade {
    pub fn to_unit_trade<'a>(
        ...
        let buy_system = universe
            .get_system_by_index(Some(index_buy_system))
            .unwrap();
        ...
        let sell_system = universe
            .get_system_by_index(Some(index_sell_system))
            .unwrap();
...
impl SearchCache {
    ...
    pub fn random_1hop_trade<'a>(
        ...
        for _ in 0..(buys.len() * sells.len()) {
            let buy = match buys.choose(&mut thread_rng()) {
                Some(v) => v,
                None => {
                    break;
                }
            ...
            let sell_option = match sell_options.choose(&mut thread_rng()) {
                Some(v) => v,
                None => {
                    continue;
                }
    ...
    pub fn get_1hop_trades<'a>(
        ...
        let insert = self.trade_cache.read().unwrap().get(&station_id).is_none();

        if insert {
            let trades =
                SearchCache::best_1hop_trades(universe, &station.state, station.search_quality);
            let cache_list = trades
                .iter()
                .map(|e| SearchCache::to_cached_trade(e, universe))
                .collect();
            ...
            trades
                .iter()
                .map(|e| e.to_unit_trade(universe, &station.state))
                .collect()
        }
    }

    fn best_1hop_trades<'a>(
        universe: &'a Universe,
        state: &PlayerState,
        ...
        let mut trade_buffer =
            ScoredCircularBuffer::new(search_quality.get_hop_width(), Sort::Descending);
    ...
    pub fn len(&self) -> usize {
        self.trade_cache.read().unwrap().len()
    }
}
