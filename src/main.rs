...
pub const SEPARATOR: &'static str =
    "-------------------------------------------------------------------";
pub const CACHE_FILENAME: &'static str =
    concat!("elite_universe_", env!("CARGO_PKG_VERSION"), ".min.json");
...
fn options() -> Options {
    ...
    opts.optopt(
        "c",
        "cargo",
        "maximum cargo capacity in tons. find this in your right cockpit panel's Cargo tab.",
        "216",
    );
    opts.optopt(
        "r",
        "range",
        "maximum laden jump range in light years.  find this in your outfitting menu.",
        "18.52",
    );
    ...
    opts.optopt(
        "q",
        "quality",
        "search quality setting [med|high|ultra]",
        "med",
    );
    opts.optopt(
        "p",
        "shipsize",
        "current ship size (small|med|large)",
        "large",
    );
    opts.optopt(
        "d",
        "debug",
        "searches to the given hop length and prints stats",
        "12",
    );
    opts.optopt(
        "C",
        "edce",
        "enables Elite Dangerous Companion Emulator integration - \
...
fn main() {
    println!("{}", SEPARATOR);
    println!(
        "Welcome to Austin's Elite Dangerous trading calculator v{}",
        VERSION
    );
...
fn run_diagnostic(universe: &mut Universe, state_in: &PlayerState, search_quality: SearchQuality) {
    ...
    println!(
        "Enumerating {} trades per station to a depth of {} hops ...",
        hop_width, depth
    );
    ...
    println!("\troute:\t\t\ttrade:");
    println!(
        "option\tpft/min\tmins\tprofit\tpft/min\tmins\tprofit\tcmdy.\tplanetary\tsystem\tstation"
    );
    ...
    for (i, result) in trades.iter().enumerate() {
        ...
        println!(
            ...
            if result.trade.unit.sell_station.is_planetary {
                "planetary"
            } else {
                "station"
            },
...
fn run_debug(
    ...
    println!(
        "{}\t{}\t{:.3}\t{:.3}",
        hops, profit_total, minutes, profit_per_min
    );
    println!("{}", SEPARATOR);
    println!("{} hops", hops);
    println!(
        "{} total profit",
        NumericUnit::new_string(profit_total, &"cr".to_string())
    );
...
fn run_search(
    ...
    println!(
        "Total routes to examine: {}",
        NumericUnit::new_string(total_routes, &"".to_string())
    );
    println!("{}", SEPARATOR);
    ...
    'route: while !quit {
        ...
        'trade: for result in results.drain(..) {
            ...
            println!(
                "hop {}:\t{}, estimated {} profit/min over next {:.0} minutes",
                i,
                now.format("%I:%M%p").trim(),
            ...
            println!(
                "buy:\t{} [{}]",
                trade.unit.buy_system.system_name, trade.unit.buy_station.station_name,
            );
            ...
            println!("");
            println!(
                "expect:\t{} profit/min from trade over {:.1} mins",
        ...
        if let Some(edce_data) = EdceData::generate_opt(&args.edce_path) {
            if let Some(price_update) = edce_data.apply_edce_adjustments(universe) {
                if price_update.station.station_id == trade.unit.sell_station.station_id {
                    let updated_active = price_update
                        .changes
                        .iter()
                    ...
                    if updated_active {
                        println!(
                            "edce:\tupdated trade commodity and {} others",
                            std::cmp::max(price_update.changes.len() - 1, 0)
                        );
                    } else {
                        println!("edce:\tupdated {} commodities", price_update.changes.len());
                    }
                    println!("");
                } else if price_update.station.station_id == trade.unit.buy_station.station_id {
                    println!(
                        "edce:\tdata received was for original buy station - {}",
                        price_update.station.station_name
                    );
                    println!("\tplease wait a few seconds after docking before completing trade");
                } else if price_update.station.station_id == trade.unit.buy_station.station_id {
                    println!(
                        "edce:\tdata received was for unknown station - {}",
                        price_update.station.station_name
                    );
